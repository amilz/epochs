        ...
        <>
            <header className="flex h-16 w-full items-center justify-between px-4 md:px-6">
                <Link className="flex items-center gap-2" href="/">
                    <Image src="/logo.svg" alt="Eclipse Noggles" width={80} height={30} />
                    <span className="text-white hidden">Epochs</span>
                </Link>
                <nav className=" items-center  md:flex">
                    {links.map((link) => (
                        <div className="lg:block hidden !mr-4 last:mr-0" key={link.href}>
                            <Link className="text-sm font-medium hover:underline" href={link.href} target={link.external ? "_blank" : "_self"}>
                                {link.title}
                            </Link>
                        </div>
                    ))}
            ...
            <div className="fixed bottom-0 left-0 z-50 flex w-full justify-around bg-black p-4 shadow-t md:hidden border-t-2">
                {
                    links.map((link) => (
                        <Link className="flex flex-col items-center gap-1" href={link.href} target={link.external ? "_blank" : "_self"} key={link.href}>
                            {link.icon}
